...
# GCP / gcloud
if [ ! -n "${BULLETTRAIN_GCLOUD_BG+1}" ]; then
  BULLETTRAIN_GCLOUD_BG=black
fi
if [ ! -n "${BULLETTRAIN_GCLOUD_FG+1}" ]; then
  BULLETTRAIN_GCLOUD_FG=white
fi
if [ ! -n "${BULLETTRAIN_GCLOUD_PREFIX+1}" ]; then
  BULLETTRAIN_GCLOUD_PREFIX="☁"
fi
...
prompt_gcloud() {
  if command -v gcloud >/dev/null 2>&1; then
    if [[ -r ~/.config/gcloud/active_config && -f ~/.config/gcloud/active_config ]]; then
      prompt_segment $BULLETTRAIN_GCLOUD_BG $BULLETTRAIN_GCLOUD_FG $BULLETTRAIN_GCLOUD_PREFIX" $(cat ~/.config/gcloud/active_config)"
    fi
  fi
}
